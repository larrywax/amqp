defmodule AMQP.Test.AMQPTest do
  use ExUnit.Case
  alias AMQP.Test.Support.{Consumer1, Consumer2, Producer1, Producer2, Producer3}
  import Mock

  setup_all do
    AMQP.ConnectionManager.start_link(%{connection_params: Application.get_env(:amqp, :amqp_connection)})

    AMQP.Producer.start_link(Application.get_env(:amqp, :producer))

    Application.get_env(:amqp, :consumers)
    |> Enum.each(&AMQP.Consumer.start_link(&1))
