defmodule QueueTest do
  ...
  test "declare queue with server assigned name", meta do
    assert {:ok, %{queue: queue, message_count: 0, consumer_count: 0}} = Queue.declare(meta[:chan])
  ...
  test "declare queue with explicitly assigned name", meta do
    ...
    assert {:ok, %{queue: ^name, message_count: 0, consumer_count: 0}} = Queue.declare(meta[:chan], name)
  ...
  test "bind and unbind queue to exchange", meta do
    ...
    assert {:ok, %{queue: ^queue, message_count: 0, consumer_count: 0}} = Queue.declare(meta[:chan], queue)
